    ...
    return new Promise((resolve, reject) => {
      this.worker.onmessage = (event) => {
        if (event.data && event.data.type === "ready") {
          ...
          this.worker = null;
          reject(new Error(event.data.message || "niimath worker failed to initialize"));
        }
      };
...
var ImageProcessor = class {
  ...
  // The optional `weight` is a base(fixed)-space region-weight image (fast engine only;
  // its dims + world frame must match `base`) that focuses the fine-stage fit on a region
  // — e.g. a brain mask — rather than whole-head features. Emitted as `-weight <img>` after
  // the base + opts and staged into MEMFS like the other file operands.
